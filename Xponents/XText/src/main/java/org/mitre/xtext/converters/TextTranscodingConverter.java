 ...
 */
public class TextTranscodingConverter implements iConvert {

    private final CharsetDetector cd = new CharsetDetector();
    private final static int IGNORE_THRESHOLD_SIZE = 1024; // 1KB
    private final static int IGNORE_THRESHOLD_CONF = 65;   // 0 to 100

    /** A converter that tries to get a decent encoding 
     *  ASCII, UTF-8 or other,  and then the payload converted or not.
     * 
     *  IF ASCII OR UTF-8
     *      accept file as is, do not convert, alter payload...
     *  ELSE   
     *      file must be read in and converted.
     * 
     *  CAVEAT:  If file is short and low-confidence for encoding detection
     *       ALSO do not convert.  Treat as a plain text file.
     */
    @Override
    public ConvertedDocument convert(java.io.File doc) throws IOException {
        ...
        // 
        if (is_ascii || ConvertedDocument.OUTPUT_ENCODING.equalsIgnoreCase(cs.getName())) {
